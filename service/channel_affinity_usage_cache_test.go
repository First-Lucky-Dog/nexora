...
// Monotonic sequence for unique cache keys; wall-clock nanos collide under
// Windows' coarse timer resolution when tests run within one tick.
var channelAffinityUsageCacheTestSeq atomic.Int64

func uniqueChannelAffinityTestKey(prefix string, t *testing.T) string {
	return fmt.Sprintf("%s_%s_%d", prefix, t.Name(), channelAffinityUsageCacheTestSeq.Add(1))
}
...
func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	ruleName := uniqueChannelAffinityTestKey("rule", t)
	usingGroup := "default"
	keyFP := uniqueChannelAffinityTestKey("fp", t)
